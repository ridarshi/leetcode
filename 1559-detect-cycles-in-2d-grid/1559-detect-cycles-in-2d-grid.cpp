...
public:
        ...
        for (int x = 0; x < 4; x++) {
            ...
            if (nrow >= 0 && nrow < m && ncol >= 0 && ncol < n &&
                ...
                if (!vis[nrow][ncol]) {
                    ...
                    // Current cell becomes the parent
                    if (dfs(nrow, ncol, i, j, vis, grid)) {
                        return true;
                    }
                }
    ...
    bool containsCycle(vector<vector<char>>& grid) {
        ...
        // Check every cell because the grid
        // can contain multiple disconnected components
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {

                if (!vis[i][j]) {

                    // Start DFS with no parent
